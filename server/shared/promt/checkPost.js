...
async function checkPost(text) {
   const PROMPT_CHECK_POST =
    `For the given text passage, return true if it is related to employment and recruitment issues, otherwise return false (only return the string true or false, without any further explanation): `

  const res = await extractData(PROMPT_CHECK_POST, text)
